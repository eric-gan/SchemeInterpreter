...
;expect (lambda (a b c) (= ((* a a) + (*b b)) (* c c)))
(define (square x) (* x x))
;expect square 

(define (radian_to_degree x) (/ (* x 180) 3.14))
;expect radian_to_degree

radian_to_degree
;expect (lambda (x) (/ (* x 180) 3.14))

(define (force m a) (* m a))
;expect force
...
(define (pow b n)
  (cond ((equal? n 0) 1)
        ...
        (else (* (square(pow b (quotient n 2))) b))
  )
)
;expect pow
...
(let ((z (lambda (x y) (+ (fib x) (fib y)))) (x 5) (y 7))
  (z x y))
;expect 18
...
(define momentum (mu (m v) (* m v)))
;expect momentum

(momentum 2 3)
;expect 6

(define work (mu (f d) (* f d)))
;expect work

(define w (lambda (m g d) (work (* m g) d)))
;expect w
