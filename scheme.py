...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('# of argument values does not match # of formal parameters')
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Invalid number of arguments to function.')
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        applied_procedure = scheme_apply(self, operands, env)
        return scheme_eval(complete_eval(applied_procedure), env)
...
def do_define_macro(expressions, env):
    ...
    target = expressions.first
    if scheme_symbolp(target):
        raise SchemeError('non-function')
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            check_procedure(scheme_eval(first, env))
            result = scheme_eval(first, env).eval_call(rest, env)
    return result
